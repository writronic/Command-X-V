...
APP_NAME="Command X-V"
SCHEME="Command X-V"

SIGNING_IDENTITY="${SIGNING_IDENTITY:?Error: SIGNING_IDENTITY not set. Export it or pass via env.}"
KEYCHAIN_PROFILE="${KEYCHAIN_PROFILE:-CommandXV}"

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
PROJECT_ROOT="$(dirname "$SCRIPT_DIR")"
XCODEPROJ="${PROJECT_ROOT}/Command-X-V.xcodeproj"

ENTITLEMENTS_MAIN="${PROJECT_ROOT}/Command X-V/Command X-V.entitlements"
...
if [[ "$ARCH" == "universal" ]]; then
    DMG_NAME="Command-X-V.dmg"
else
    DMG_NAME="Command-X-V-${ARCH}.dmg"
fi

DMG_OUTPUT="${OUTPUT_DIR}/${DMG_NAME}"

echo "╔══════════════════════════════════════════════════════════════╗"
echo "║  Command X-V Build & DMG — ${ARCH}                         "
echo "╚══════════════════════════════════════════════════════════════╝"
echo ""
...
if [[ "$ARCH" == "universal" ]]; then
    ...
    lipo -info "$APP_PATH/Contents/MacOS/Command X-V"
else
...
# Main app (top-level, last)
echo "  Signing Command X-V.app..."
codesign --force --options runtime --timestamp \
    --sign "$SIGNING_IDENTITY" \
    "${CODESIGN_EXTRA[@]}" \
...
rm -f "$DMG_OUTPUT"
rm -f "${OUTPUT_DIR}/${APP_NAME} "*.dmg 2>/dev/null || true
